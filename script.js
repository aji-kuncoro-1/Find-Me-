(() => {
  ...
  canvas.addEventListener('touchstart', e => {
    ...
    for (const t of e.changedTouches) {
      ...
      if (dA < dB) {
        if (dA < 90 && soulA.touchId === null) { 
          soulA.isDragging = true; soulA.touchId = t.identifier; soulA.tx = p.x; soulA.ty = p.y + LIFT; 
          playMusic(); // <-- Pemanggilan Audio
        }
        else if (dB < 90 && soulB.touchId === null) { 
          soulB.isDragging = true; soulB.touchId = t.identifier; soulB.tx = p.x; soulB.ty = p.y + LIFT; 
          playMusic(); // <-- Pemanggilan Audio
        }
      } else {
        if (dB < 90 && soulB.touchId === null) { 
          soulB.isDragging = true; soulB.touchId = t.identifier; soulB.tx = p.x; soulB.ty = p.y + LIFT; 
          playMusic(); // <-- Pemanggilan Audio
        }
        else if (dA < 90 && soulA.touchId === null) { 
          soulA.isDragging = true; soulA.touchId = t.identifier; soulA.tx = p.x; soulA.ty = p.y + LIFT; 
          playMusic(); // <-- Pemanggilan Audio
        }
      }
  ...
  canvas.addEventListener('mousedown', e => {
    ...
    if (dA < dB && dA < 70) { 
      soulA.isDragging = true; activeNode = soulA; 
      playMusic(); // <-- Pemanggilan Audio
    }
    else if (dB < 70) { 
      soulB.isDragging = true; activeNode = soulB; 
      playMusic(); // <-- Pemanggilan Audio
    }
  });
